...
import ConfigModel from "./components/ConfigModal";
...
function App() {
  ...
  useEffect(() => {
    const onLoad = async () => {
      ...
      const uniContarct = getUniContract();
      setUniContract(uniContarct);
    };
    onLoad();
  }, []);
  ...
  const getSwapPrice = (inputAmount) => {
    ...
    const swap = getPrice(
      ...
      // console.log(Number(data[1]).toFixed(5))
      setOutputAmount(Number(data[1]).toFixed(5));
      setRatio(data[2]);
      setLoading(false);
    });
  };
